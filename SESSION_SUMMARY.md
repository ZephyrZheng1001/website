...
### ????
- **????**?????"?????Zephyr?"?????"??????????????????????????????????????????????[???](/#/about)"
- **????**????????????????????"2026 ???? ? ???? 2026-08-05 23:15:42"
- **Markdown ????**?`.article-content` ??? table/thead/th/td ???????????????????
- **Mermaid ??**??? ` ```mermaid ` ??????????? mermaid.ink ????? mermaid ?????????????? JS ?
- **???????**?Admin.vue ? editorForm ???/openWrite/editArticle ????? `subcategory` ? `study_status` ????????????????

### ?????

#### ?13?PowerShell ?????????
- `git show HEAD:file > file` ?? UTF-16 LE (BOM `FF FE`) ??
- Python `open(encoding='utf-8')` ?????? `UnicodeDecodeError`
- **????**??? PowerShell `Set-Content -Encoding UTF8` ??

#### ?14?PowerShell ? Python ????????
- PowerShell ? `-replace`?`Replace()` ?? here-string ???????? (`
` vs `
`)???????????????
- ??????????? unchanged??????
- **????**?? Python `rb` ????????????????
...
#### ?15?Mermaid ?????????
- ??1?CDN ???? `mermaid.min.js`?? `mermaid.render()` ?? ? jsdelivr CDN ?????
- ??2?? unpkg CDN ? ?????
- ??3?? `mermaid.run()` API ?? `<div class="mermaid">` ? ??????
- ??4?? marked code renderer ??? `lang==='mermaid'` ?? `<div class="mermaid">` ? ????????? git ?????????
- ?????marked code renderer ????? `<img src="https://mermaid.ink/img/...">`??? mermaid.ink ????????????????????? JS ??????

#### ?16?Vue SFC ? JS ????? HTML ?????
- `return '<div class="mermaid-container"><img src="..." /></div>'` ? HTML ??????? JS ????????
- ???? `Missing semicolon`
- **????**?JS ??????????HTML ???????`return '<div class="..."><img src="..." /></div>'`?

#### ?17?`git checkout` ? sandbox ? Permission denied
- `.git/index.lock` ?????? `git checkout` ??
- **????**?? `git show <commit>:<path> > file` ????

#### ?18?PowerShell + SSH + Python ?????? Python ????
- ? PowerShell ??? SSH ???? Python ? `-c` ???????`$` ?????
- ? `SyntaxError: unterminated string literal`
- **??**???? PowerShell SSH ????? Python ??